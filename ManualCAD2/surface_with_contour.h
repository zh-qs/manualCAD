...
namespace ManualCAD
{
	...
	public:
		...
		SurfaceWithBezierContour(const Matrix4x4& model, const Texture* texture) : Renderable(model), ebo(), contour_ebo(), texture(texture) { init_additional_buffers(); }
		SurfaceWithBezierContour(const Texture* texture) : Renderable(), ebo(), contour_ebo(), texture(texture) { init_additional_buffers(); }
	...
	public:
		...
		SurfaceWithDeBoorContour(const Matrix4x4& model, const Texture* texture) : Renderable(model), ebo(), contour_ebo(), texture(texture) { init_additional_buffers(); }
		SurfaceWithDeBoorContour(const Texture* texture) : Renderable(), ebo(), contour_ebo(), texture(texture) { init_additional_buffers(); }
	...
	class NURBSWithDeBoorContour : public Renderable {
		size_t patch_indices_count = 0;
		size_t contour_indices_count = 0;
		int patches_x = 0, patches_y = 0;
		ElementBuffer ebo, contour_ebo;
		...
		void init_additional_buffers() {
			...
			weight_vbo.init();
			weight_vbo.bind();
			weight_vbo.attrib_buffer(1, 1, GL_FLOAT);
			vao.unbind();
		}
	public:
		Vector4 contour_color = { 1.0f,1.0f,1.0f,1.0f };
		...
		NURBSWithDeBoorContour(const Matrix4x4& model, const Texture* texture) : Renderable(model), ebo(), contour_ebo(), texture(texture) { init_additional_buffers(); }
		NURBSWithDeBoorContour(const Texture* texture) : Renderable(), ebo(), contour_ebo(), texture(texture) { init_additional_buffers(); }
		...
		// Points should be aligned in a grid of points_x columns and points_y rows
		inline void reset_ebos() { ebos_set = false; }
		void set_data(const std::vector<Vector3>& points, const std::vector<float>& weights, int patches_x, int patches_y);
		void render(Renderer& renderer, int width, int height, float thickness = 1.0f) const override;
		void dispose() override { Renderable::dispose(); ebo.dispose(); contour_ebo.dispose(); }
	};
}