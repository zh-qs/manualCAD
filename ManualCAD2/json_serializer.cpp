...
namespace ManualCAD
{
	MG1::Float3 from_vector3(const Vector3& vec)
	{
		return { vec.x,vec.y,vec.z };
	}

	MG1::Float2 from_vector3_2(const Vector3& vec)
	{
		return { vec.x,vec.y };
	}

	Vector3 from_float3(const MG1::Float3& f)
	{
		return { f.x,f.y,f.z };
	}

	MG1::PointRef get_point_ref_to(const ObjectController& controller, const Point& point)
	{
	...
	JSONSerializer::JSONSerializer(ObjectController& controller, Camera& camera) : controller(controller), camera(camera)
	{
		object_count = controller.objects.size();
	}

	void JSONSerializer::serialize(const std::filesystem::path& filepath)
	{
	...
	void JSONSerializer::add_point(const Point& point, int idx)
	{
		MG1::Point p;
		p.name = point.name;
	...
	void JSONSerializer::add_torus(const Torus& torus, int idx)
	{
		MG1::Torus t;
		t.name = torus.name;
	...
	void JSONSerializer::add_bezier_c0_curve(const BezierC0Curve& curve, int idx)
	{
		MG1::BezierC0 c;
		c.name = curve.name;
		for (auto* p : curve.points)
			c.controlPoints.push_back(get_point_ref_to(controller, *p));
		c.SetId(idx);
		MG1::Scene::Get().bezierC0.push_back(c);
	}

	void JSONSerializer::add_bezier_c2_curve(const BezierC2Curve& curve, int idx)
	{
		MG1::BezierC2 c;
		c.name = curve.name;
		for (auto* p : curve.points)
	...
	void JSONSerializer::add_interpolation_spline(const InterpolationSpline& spline, int idx)
	{
	...
	void JSONSerializer::add_bicubic_bezier_c0_surface(const BicubicC0BezierSurface& surf, int idx)
	{
			...
			for (int i = 0; i < surf.patches_x; ++i)
			{
				...
				for (int y = 0; y < 4; ++y)
					for (int x = 0; x < 4; ++x)
					{
						...
						MG1::PointRef pref = get_point_ref_to(controller, *surf.points[array_idx]);
	...
	void JSONSerializer::add_bicubic_bezier_c2_surface(const BicubicC2BezierSurface& surf, int idx)
	{
	...
	void JSONSerializer::add_bicubic_nurbs_c2_surface(const BicubicC2NURBSSurface& surf, int idx)
	{
		// TODO
	}

	void JSONSerializer::deserialize(const std::filesystem::path& filepath)
	{
		MG1::Scene& scene = MG1::Scene::Get();
